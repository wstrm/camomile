...
// IDWithPrefixGenerator generates random IDs with the provided ID as a prefix.
//
...
//		* and so on...
func IDWithPrefixGenerator(prefix ID) chan ID {
	ch := make(chan ID)

	go func(ch chan ID) {
		for i := 1; i < IDLength; i++ {
			ch <- NewIDWithPrefix(prefix, i)
		}
...
// NewIDWithPrefix creates a new ID with the provided prefix. The prefix length
// is set in bits with the i parameter.
func NewIDWithPrefix(prefix ID, i int) ID {
	j := (i - 1) / 8 // Byte position.

	// Create new instance of ID every iteration.
	id := NewID()

	// Copy prefix of input prefix ID into the new ID.
	copy(id[:j], prefix[:j])
